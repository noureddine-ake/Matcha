...
function VerifyEmailUpdateContent() {
  ...
  useEffect(() => {
    ...
    const verifyEmail = async () => {
      try {
        const response = await api.post('/email-update/verify', { token });
        setStatus('success');
        ...
        setStatus('error');
        setMessage(axiosError.response?.data?.error || 'Verification failed. The link may be invalid or expired.');
      }
    };
